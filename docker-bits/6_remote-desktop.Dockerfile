...
RUN \
    code --install-extension ms-python.python --no-sandbox --user-data-dir $NB_USER && \
    code --install-extension ms-ceintl.vscode-language-pack-fr --no-sandbox --user-data-dir $NB_USER && \
    # Fix permissions
    fix-permissions $HOME/.vscode/extensions/ && \
    # Cleanup
    clean-layer.sh
