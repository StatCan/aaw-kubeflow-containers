...
RUN wget "https://go.dev/dl/${GO_VERSION}.tar.gz" -O /opt/godevtools.tar.gz
RUN tar -xf /opt/godevtools.tar.gz
RUN rm /opt/godevtools.tar.gz \
RUN PATH="$PATH:/opt/go/bin/"
